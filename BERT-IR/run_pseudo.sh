...
TASK_NAME="TDT3"
DATA_DIR="data"
OUTPUT_EXPDIR="exp_conf/TDT2"
stage=0
train_set=conf_train
test_set=conf_test_short

set -euo pipefail

if [ $stage -le 0 ]; then 
    # text (train, test_short, test_long)
    #python preprocess.py --output_dir $DATA_DIR --task_name $TASK_NAME \
    #                     --is_training false --is_short false --is_spoken false
    # spoken (train, test_short, test_long)
    python preprocess.py --output_dir $DATA_DIR --task_name $TASK_NAME \
                         --is_training false --is_short false --is_spoken true
    stage=10000
fi
