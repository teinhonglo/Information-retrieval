...
def IDF(qry, doc):
    qry_len = compLenAcc(qry)
    doc_len = compLenAcc(doc)
    doc_freq = docFreq(doc)
    num_docs = len(list(doc.keys())) + 1
    qry_new = {q_id : {q_wid : np.log2(num_docs / (1 + doc_freq[q_wid][0]))
                for q_wid, q_wc in q_content.items()} for q_id, q_content in qry.items()}
    doc_new = {d_id : {d_wid : np.log2(num_docs / (1 + doc_freq[d_wid][0]))
