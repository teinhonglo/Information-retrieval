...
def create_single_model(maxlen, word_rep):
    maxlen = None
    num_u = 96 
    reduction_d_a = 64
    reduction_r = 1
    ...
    for qry_idx, qry_vec in enumerate(qry_emb):
        ...
        for doc_idx, doc_vec in enumerate(doc_emb):
            ...
            if (len(qry_test) % 16 == 0) or (doc_idx == doc_length):
                qry_test = pad_sequences(np.array(qry_test), dtype='float32')
                doc_test = pad_sequences(np.array(doc_test), dtype='float32')
