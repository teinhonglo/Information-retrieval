...
def create_model(J=3, WORD_DEPTH=100, K=300, L=128):
    ...
    qd_conv = Convolution1D(K, FILTER_LENGTH, padding = "same", input_shape = (None, WORD_DEPTH), activation = "tanh", name = "conv")
    qd_max = Lambda(lambda x: backend.max(x, axis = 1), output_shape = (K, ), name="max_pooling")
    ...
    query_conv = qd_conv(query)
    # Next, we apply a max-pooling layer to the convolved query matrix. 
    query_max = qd_max(query_conv) # See section 3.4.
    ...
    pos_doc_conv = qd_conv(pos_doc)
    neg_doc_convs = [qd_conv(neg_doc) for neg_doc in neg_docs]        
    
    pos_doc_max = qd_max(pos_doc_conv)
    neg_doc_maxes = [qd_max(neg_doc_conv) for neg_doc_conv in neg_doc_convs]
